...
function ThongTinBaiViet() {
    ...
    const [posts, setPosts] = useState([]); // Khởi tạo mảng rỗng
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        // Gọi API từ Laravel (Port 8000)
        axios.get('http://localhost:8000/api/recipes')
            .then(response => {
                setPosts(response.data); // Đưa dữ liệu từ DB vào posts
                setLoading(false);
            })
            .catch(error => {
    ...
    if (loading) return <div className="text-center p-10">Đang tải bài viết...</div>;
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-4">
                ...
                {/* Hiển thị danh sách bài viết từ DB */}
                <BaiViet data={posts}/>
            </div>

            {/* --- CỘT PHẢI: GỢI Ý - Giữ nguyên --- */}
