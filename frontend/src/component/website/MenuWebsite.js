            ...
            <nav className="bg-white shadow-sm fixed w-full z-50 top-0 h-16 border-b border-gray-200">
                <div className="container mx-auto px-4 h-full flex justify-between items-center max-w-7xl">
                    ...
                    <div className="flex-1 max-w-xl mx-4 relative group z-50">
                        ...
                        {/* Filter Panel (Giữ nguyên code cũ) */}
                        <div id="filterPanel" className={`${isFilterOpen ? 'block' : 'hidden'} absolute top-full left-0 mt-3 w-full bg-white rounded-xl shadow-2xl border border-gray-100 p-5 animate-fade-in-down`}>
                            ...
                            <div className="grid grid-cols-2 gap-6">
                                {/* Nội dung filter (Vùng miền, nguyên liệu...) giữ nguyên như cũ */}
                                {/* ...Code filter cũ của bạn... */}
                                <div>
                                ...
                                {/* ... (Giữ nguyên các phần filter khác để code gọn) ... */}
                            </div>
                            
                            <div className="flex justify-between items-center mt-4 pt-2 border-t border-gray-100">
                                <button onClick={toggleFilter} className="text-xs text-gray-500 hover:text-gray-800 underline">Xóa bộ lọc</button>
                                <button onClick={toggleFilter} className="bg-red-600 text-white text-sm font-bold px-6 py-2 rounded-lg hover:bg-red-700 shadow-lg shadow-red-200 transition">Áp dụng</button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        {/* --- PHÂN QUYỀN: Kiểm tra User --- */}
                        {user ? (
                            /* TRƯỜNG HỢP 1: ĐÃ ĐĂNG NHẬP (Hiện Avatar & Menu Dropdown) */
                            <div className="relative">
                                ...
                                >
                                    <img 
                                        ...
                                        alt="User Avatar" 
                                        className={`w-9 h-9 rounded-full border-2 ${isUserMenuOpen ? 'border-red-500 shadow-md' : 'border-gray-200'} transition-all object-cover`} 
                                    />
                                </button>
